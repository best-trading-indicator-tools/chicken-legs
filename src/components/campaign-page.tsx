...
export default function CampaignPage() {
  ...
  function selectSlot(id: string) {
    ...
    setView(id.includes('quad') ? 'front' : 'back');
  }
  ...
  return <>
    ...
    <main>
      {paymentStatus && <div className={`payment-notice ${paymentStatus === 'accepted' ? 'success' : ''}`} role="status">
        <ShieldCheck size={20} /><span>{paymentStatus === 'accepted' ? 'Payment confirmed. Your brand has officially joined leg day.' : paymentStatus === 'cancelled' ? 'Checkout cancelled. No spot has changed hands.' : paymentStatus === 'refunding' ? 'This payment could not claim the spot. Your refund is being processed.' : paymentStatus === 'expired' ? 'This checkout expired. Choose a spot to try again.' : 'Checking your payment. Your sponsorship appears after confirmation.'}</span><button aria-label="Dismiss payment notification" onClick={() => setPaymentStatus(null)}><X size={16} /></button>
      </div>}
      <section className="hero" aria-labelledby="hero-title">
        ...
        <div className="model-column">
          <div className="model-topline"><span><Crosshair size={13} /> THE PRIME REAL ESTATE</span><span>01—06</span></div>
        ...
        <aside className="sponsor-panel" id="sponsorships" aria-labelledby="sponsorship-title">
          ...
          <div className="panel-heading"><h2 id="sponsorship-title">PICK YOUR<br />PIECE OF LEG.</h2><p>Six spots. One unforgettable placement.</p></div>
