...
For quick look-development renders, append `-- --quick`. The local tool downloaded during production lives in ignored `output/blender-tool/Blender.app/Contents/MacOS/Blender`.
...
- `public/models/placements.json`: six sponsor positions, outward normals and indicative decal dimensions.
- `public/images/legs-front.png`, `legs-back.png`, `legs-preview.png`: transparent full-body fallback / review renders.

Blender source coordinates: Z-up, facing −Y. Browser export: Y-up, facing +Z, anatomical left +X. Native leg anchors are named `left_quad`, `right_quad`, `left_hamstring`, `right_hamstring`, `left_calf`, `right_calf`. Their local +Z points outward from the skin. The browser derivative shifts the sole down 0.009 units to floor height zero. Placement metadata uses browser coordinates.
